...
import { searchKnowledge, getClubDocuments, uploadDocument, deleteDocument, getKnowledgeStats } from '../services/api';
...
export default function KnowledgePage() {
  ...
  const fetchDocs = async () => {
    ...
    try {
      ...
      const [docsRes, statsRes] = await Promise.allSettled([
        getClubDocuments(activeClub.id),
        getKnowledgeStats(activeClub.id),
      ]);
  ...
  const handleDeleteDocument = async (docId) => {
    if (!window.confirm('Are you sure you want to remove this document and delete its FAISS vector embeddings?')) return;
    try {
      const res = await deleteDocument(activeClub.id, docId);
      if (res.success) {
        setUploadSuccess('Document removed and FAISS vector index re-synchronized!');
        fetchDocs();
      }
    } catch (err) {
  ...
  const sampleQuestions = [
    ...
    'What is the budget reimbursement policy for student travel?',
  ];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/90 shadow-xs">
        <div>
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-emerald-100 text-emerald-800 flex items-center gap-1">
              <Database className="w-3 h-3 text-emerald-700" />
              FAISS Vector Database
            </span>
            <span className="text-xs text-slate-500 font-mono">
              {stats?.total_vectors !== undefined ? (
                `${stats.total_vectors} Vectors Indexed • ${stats.dimension}D Dense Embeddings • ${stats.index_type}`
              ) : (
                'Facebook AI Similarity Search (FAISS) • 384 Dim'
              )}
            </span>
          </div>
          ...
          <p className="text-sm text-slate-600 mt-1">
            Dense vector similarity search powered by FAISS. Searches historical records, SOPs, and guidelines with exact citations.
          </p>
        </div>
          ...
          <CardContent className="p-6 space-y-4">
            ...
            {searchResult.citations?.length > 0 && (
              <div className="pt-4 border-t border-emerald-100 space-y-2">
                <span className="text-xs font-bold text-slate-600">FAISS Retrieved Citations:</span>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {searchResult.citations.map((c, idx) => (
                    <div
                      key={idx}
                      className="p-3 bg-white border border-emerald-200/80 rounded-xl text-xs space-y-1 shadow-2xs"
                    >
                      <div className="flex items-center justify-between gap-1.5 text-emerald-800 font-bold truncate">
                        <div className="flex items-center gap-1.5 truncate">
                          <FileText className="w-3.5 h-3.5 shrink-0" />
                          <span className="truncate">{c.document_name}</span>
                        </div>
                        {c.confidence_pct !== undefined && (
                          <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-800 shrink-0 font-bold">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          {stats?.total_vectors !== undefined && (
            <span className="text-xs text-slate-500 font-medium">
              {stats.total_vectors} chunks embedded in FAISS
            </span>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {documents.map((doc) => (
            <div
              key={doc.id}
              className="p-4 bg-white border border-slate-200 rounded-xl flex items-start justify-between gap-3 hover:border-emerald-300 transition-colors shadow-2xs group"
            >
              <div className="flex items-start gap-3 min-w-0 flex-1">
                <div className="w-9 h-9 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center shrink-0 group-hover:bg-emerald-50 group-hover:text-emerald-700 transition-colors">
                  <FileText className="w-4 h-4" />
                </div>
                <div className="min-w-0 flex-1">
                  <h3 className="font-bold text-xs text-slate-900 truncate">{doc.name}</h3>
                  <p className="text-[11px] text-slate-500 mt-0.5 font-mono">
                    {doc.chunk_count} chunks indexed in FAISS
                  </p>
              ...
              {isManagement && (
                <button
                  ...
                  title="Remove document and FAISS vector index"
                  className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-red-600 transition-opacity p-1.5 rounded-lg hover:bg-red-50 cursor-pointer"
                >
